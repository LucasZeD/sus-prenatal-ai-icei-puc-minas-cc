...
def save_figure(fig: plt.Figure, out_dir: Path, stem: str, dpi: int = 150) -> dict[str, str]:
    ...
    for ext in ("png", "pdf"):
        ...
        paths[ext] = str(p.relative_to(REPORT_DIR)).replace("\\", "/")
    plt.close(fig)
    return paths
...
def write_dashboard(path: Path, stamp: str, figures: dict[str, dict[str, str]], metrics: dict[str, Any]) -> None:
    ...
    for key in sorted(figures.keys()):
        ...
        rows.append(f'<h3>{key}</h3><img src="../../{png}" style="max-width:1000px;width:100%;">')
    html = f"""<!doctype html>
<html lang="pt-br">
<head>
  <meta charset="utf-8" />
...
def main() -> int:
    ...
    parser.add_argument("--benchmark-csv", type=Path, default=TESTS_DIR / "dataset" / "prenatal_sus_benchmark.csv")
    ...
    gateway_csv = args.gateway_csv or (TESTS_DIR / "06_gateway_privacidade" / "results" / stamp / "gateway_results.csv")
    out_dir = args.out_dir or (REPORT_DIR / "figures" / stamp)
    dashboard_path = REPORT_DIR / "dashboard" / stamp / "index.html"
