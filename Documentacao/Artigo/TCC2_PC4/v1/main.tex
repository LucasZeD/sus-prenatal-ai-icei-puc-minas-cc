...
Essa dualidade pode ampliar esforço de registro durante as consultas, podendo comprometer o tempo de contato direto e humanizado com a paciente \cite{Viellas2014PreNatal, Rajpurkar2022}.
...
Aplicando essa premissa à arquitetura, o modelo foi ancorado exclusivamente em cartilhas e manuais oficiais do Ministério da Saúde, empregando particionamento (\textit{chunking}) semântico e hiperparâmetros conservadores para priorizar aderência documental em detrimento da fluidez generativa.
...
O acervo inclui edições de 2010--2025; para mitigar conflitos no RAG, adotou-se re-ranqueamento por \textit{Maximal Marginal Relevance} (MMR) \cite{CarbonellGoldstein1998MMR}, priorizando relevância e diversidade de \textit{chunks} sem redundância.
...
\begin{table}[htbp]
  ...
  \makebox[\textwidth][c]{
    \begin{tabularx}{1.10\textwidth}{l X X X}
      ...
      RF05 & Alucinação em conduta clínica. & RAG restrito a manuais & Respostas embasadas em protocolo oficial \\
      US04 & Automação indevida & Bloqueio de persistência sem aprovação & Decisão final do profissional \\
...
Este sistema de avaliação foi estruturado em conformidade com práticas da indústria para avaliação de agentes de IA, como a do \textit{Microsoft Copilot Studio Evaluation Framework} \cite{MicrosoftCopilotEval2026}.
...
O Gemini-3.1-Flash-Lite foi utilizado apenas como linha de base externa de comparação, sem integrar a arquitetura proposta proposta nem alterar sua premissa local.
...
A ativação do RAG no modelo local elevou a APR de 53,1\% para 57,3\% e reduziu a TFR (Taxa de Erro de Conduta) de 20,0\% para 10,0\%.
...
Os quatro itens excluidos da avaliação demandam revisão qualitativa da equipe clínica e não entram no denominador da Tabela~\ref{tab:apr_tfr_providers}.
