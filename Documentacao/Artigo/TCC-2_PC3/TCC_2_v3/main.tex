...
\title{Digitalização da Caderneta da Gestante: Uma Arquitetura de IA Responsável para Apoio à Decisão Clínica no Pré-Natal}
...
\begin{resumo}
  A documentação de pré-natal no Brasil é fragmentada entre registros físicos e digitais, e o uso de Modelos de Linguagem de Grande Escala no setor público levanta riscos de privacidade e alucinação.
  ...
  Para garantir a segurança clínica, exige-se revisão médica antes de salvar qualquer registro (\textit{human-in-the-loop}). A contribuição principal é a implementação de um protótipo de alta fidelidade replicável e seguro.
...
Essa dualidade impõe maior carga cognitiva durante as consultas, podendo comprometer o tempo de contato direto e humanizado com a paciente \cite{ThemeFilha2014, Rajpurkar2022}.
...
(iii) adote um \textit{gateway} de privacidade dedicado e um fluxo \textit{human-in-the-loop} para assegurar supervisão médica \cite{FrameworkGovBR2025};
...
\begin{itemize}
  	\item Como estruturar uma arquitetura local em contêineres que reduza o risco de exposição de áudio ou dados textuais sensíveis a serviços externos de geração e transcrição?
...
Este projeto possui como objetivo geral especificar, implementar e avaliar uma plataforma digital \textit{on-premise} para apoio documental ao pré-natal, integrando \textit{LLMs} fundamentadas via \textit{RAG}, mascaramento de PII para apoiar o registro e a tomada de decisão clínica, com ênfase em explicabilidade, privacidade por desenho, auditabilidade e alinhamento às diretrizes públicas de saúde.
...
\begin{enumerate}
  ...
  \item Desenvolver um protótipo funcional voltado à consulta assistida de informações de pré-natal, mantendo revisão humana obrigatória antes de qualquer uso da resposta gerada;
  \item Implementar um modulo de anonimização dedicado ao mascaramento e sanitização de PII brasileiras;
  \item Estruturar a recuperação de conhecimento via (\textit{RAG}) com base em documentos públicos oficiais;
  \item Avaliar o desempenho do protótipo por meio de métricas de recuperação, aderência automática de respostas, testes sintéticos de PII, verificação de escopo e análise de latência.
...
\subsection{Hardware}
A escolha do modelo obedeceu ao critério de viabilidade operacional local, sendo executado executado em ambiente local com CPU Intel i7-14700K, 32GB de RAM e GPU Nvidia RTX 5060 Ti com 16GB de VRAM.
A escolha do Qwen3.5 9B quantizado em Q4 decorreu da compatibilidade com a VRAM disponível, onde modelos acima de $\sim$14\,GB de VRAM após quantização Q4 exigiriam \textit{offloading} para RAM/CPU, elevando a latência (TTFT) acima do tolerável em consulta.
...
A execução ocorre em contêineres \textit{Docker}, assegurando \textit{sandboxing} e isolamento de rede entre \textit{frontend}, \textit{backend} e dados, reduzindo a exposição de PII em caso de comprometimento.

Para transcrição, integra-se o \textit{Faster-Whisper} com processamento local e descarte imediato do stream de áudio, evitando o envio a terceiros (LGPD, art. 33) e reduzindo a superfície de vazamento.
...
Como base documental, as Cartilhas de Atenção Pré-Natal do Ministério da Saúde foram vetorizadas como fonte de verdade (\textit{ground truth}) (Tabela~\ref{tab:cartilhas_sus}).
O acervo inclui edições de 2010--2025; para mitigar conflitos no RAG, adotou-se re-ranqueamento por \textit{Maximal Marginal Relevance} (MMR), priorizando relevância e diversidade de \textit{chunks} sem redundância.
...
A seleção do modelo obedeceu a um critério de viabilidade operacional on-premise, com 16GB de VRAM dedicados e janela de contexto de 16.384 \textit{tokens} para RAG.
Excluindo modelos cujo consumo estimado de VRAM superasse ~12-14GB após quantizaćão Q4, de modo a evitar off-loading de memoria o que impactaria na latencia de inferência, levando em consideração que flutuações de espaço dependem diretamente da arquitetura de \textit{hardware} e de otimizações de \textit{drivers} locais \cite{VRAMCalculator}.
...
\begin{table}[H]
  ...
  \begin{tablenotes}
    \footnotesize
    \item[*] Cálculo de VRAM estimado baseado em https://apxml.com/tools/vram-calculator.
  \end{tablenotes}
  \end{threeparttable}
\end{table}

Para a escolha entre os candidatos compatíveis com 16GB de VRAM, consultou-se o comparativo público LLMBase \cite{benchmarks_llm}, que agrega scores de benchmarks (GPQA, HLE, índices compostos, etc.) para avaliação dos modelos Qwen3 14B e Qwen3.5 9B.

Embora esses resultados reflitam inferência em infraestrutura de larga escala de API e não reproduzam a arquitetura local escolhida, indicam que o Qwen3.5 9B supera o Qwen3 14B em GPQA ( 80,6\% vs 60,4\% ) e no índice composto de inteligência (32,4 vs 16,2), o que corrobora a preferência pelo menor modelo que surpreendeu apresentando melhor desempenho em racioncíno nos benchmarks agradados consultados.
...
(iii) agente Escriba para transcrição e extração estruturada;
(iv) motor RAG ancorado nas cartilhas do Ministério da Saúde;
e (v) assistente clínico local de apoio à decisão.
...
\begin{itemize}
  ...
  \item \textbf{Recuperação (Retriever):} a busca retorna os trechos relevantes e aplica re-ranqueamento por \textit{Maximal Marginal Relevance} (MMR) para mitigar redundâncias e conflitos temporais entre edições dos manuais.
...
Para cumprir a restrição de segurança clínica, adota-se o \textit{Human-in-the-Loop}, onde nenhum dado sugerido pela IA é gravado no banco de dados sem a confirmação explícita do profissional (US04), garantindo o controle total sobre o registro.
A principal ameaça à validade interna é a geração de conteúdo não suportado por protocolo (alucinação), mitigada por RAG restrito a diretrizes oficiais e \textit{Human-in-the-Loop}.
...
\begin{table}[htbp]
  ...
  \scriptsize
  \makebox[\textwidth][c]{
    \begin{tabularx}{1.10\textwidth}{l X X X}
      \toprule
      ...
      RF05 & Alucinação em conduta clínica. & RAG restrito a manuais & Respostas embasada em protocolo oficial \\
      US04 & RPA indevido e não responsabilidade & Bloqueio de persistência sem aprovação & Decisão final do profissional \\
      US13 & Exposição de PII ao modelo & \textit{Gateway} de anonimização & Dados de transição anonimizados \\
      US05 & Atraso na identificação de alto risco & Alertas por limiares das cartilhas & Sinalização visual imediata na consulta \\
...
Este sistema de avaliação foi estruturado em conformidade com práticas da indústria para avaliação de agentes de IA, espelhando frameworks corporativos como o \textit{Microsoft Copilot Studio Evaluation Framework} \cite{MicrosoftCopilotEval2026}.

%---------------------------------------------------------------------------------------------------------------------------------------------------------------------

\subsection{Recuperacao Semântica}

O módulo de recuperação foi avaliado isoladamente com \textit{top-k}=6 e expansão de consulta ativada.
Foram utilizadas três métricas: \textit{Hit@6}, que verifica se o documento-fonte esperado aparece entre os seis primeiros \textit{chunks}; \textit{Mean Reciprocal Rank} (MRR), que pondera a posição do primeiro documento correto; e \textit{phrase\_recall}, que mede a presença das frases esperadas nos trechos recuperados.
...
O melhor desempenho em itens difíceis não deve ser interpretado como maior facilidade clínica, mas como efeito provável da especificidade lexical de consultas sobre alto risco, que tendem a produzir assinaturas vetoriais mais discriminativas. Em contraste, perguntas de rotina pré-natal competem com trechos semanticamente próximos de diferentes manuais, reduzindo a precisão do ranqueamento.

%---------------------------------------------------------------------------------------------------------------------------------------------------------------------

\subsection{Geração de Resposta}

A geração foi avaliada por meio da Taxa de Aprovação de Respostas (APR), calculada sobre itens elegíveis à avaliação automática, e da Taxa de Falha em Perguntas Distratoras (TFR), calculada sobre os itens \textit{trap}. O modelo local Qwen3.5 9B foi executado com e sem RAG para medir o ganho incremental da recuperação.
O Gemini-3.1-Flash-Lite foi utilizado apenas como linha de base externa de comparação, sem alterar a proposta \textit{on-premise} da arquitetura.
...
\begin{table}[H]
  ...
  \begin{tabular}{lccc}
  	\toprule
  	\textbf{Provedor}           & \textbf{Modo RAG} & \textbf{APR (\%)} & \textbf{TEC (\%)} \\
  	\midrule
  	Qwen3.5 9B (Ollama)         & Off               & 53,1\% (51/96)    & 20,0\% (2/10)     \\
	  Qwen3.5 9B (Ollama)         & On                & 57,3\% (55/96)    & 10,0\% (1/10)     \\
  	Gemini-3.1-Flash-Lite       & On                & 61,5\% (59/96)    & 0,0\% (0/10)      \\
  	\bottomrule
  \end{tabular}
  \end{table}
...
Os cinco itens excluidos da avaliação demandam revisão qualitativa da equipe clínica e não entram no denominador da Tabela~\ref{tab:apr_tfr_providers}.

%---------------------------------------------------------------------------------------------------------------------------------------------------------------------

\subsection{Privacidade, escopo e latência}

O modulo de anonimização foi submetido a 50 frases sintéticas, totalizando 76 entidades sensíveis avaliadas, incluindo CPF, Cartão Nacional de Saúde (CNS), e-mail, telefone, nomes completos e cadeias numéricas longas.
...
\begin{table}[H]
  ...
  \begin{tabular}{lcc}
	...
	\midrule
  	...
  	CPF                                & 12                           & 0,0\%                   \\
  	E-mail                             & 12                           & 0,0\%                   \\
  	Nome Completo			   & 21                           & 0,0\%                   \\
  	Cadeias Numéricas Longas           & 8                            & 0,0\%                   \\
  	Telefone                           & 12                           & 0,0\%                   \\
	\midrule
  	\textbf{Total Global}              & \textbf{76}                  & \textbf{0,0\%}          \\
  	\bottomrule
  \end{tabular}
  \end{table}
...
%Estes fatores ampliam o ambiente de testes para uma nova configuração de hardware com espaço de VRAM a fim de comportar mais contexto e possibilitar melhor qualidade de recuperação.
...
As limitações do estudo decorrem do próprio escopo experimental. A avaliação realizada mede viabilidade arquitetural, privacidade, recuperação documental, aderência automática e comportamento funcional em casos sintéticos; ela não mede eficácia assistencial. Não houve avaliação com gestantes, profissionais de saúde ou dados clínicos reais, logo, os resultados não permitem inferir redução efetiva de carga cognitiva, usabilidade em consulta ou impacto sobre desfechos de cuidado. Essa limitação é compatível com a pergunta de pesquisa adotada, que prioriza viabilidade técnica e segurança arquitetural, mas impede conclusões sobre adoção clínica real.

Como trabalhos futuros, recomenda-se refinar a indexação por metadados de versão dos manuais, implementar barreiras determinísticas de escopo antes da geração, avaliar o sistema em áudio clínico simulado, submeter o protótipo a estudos com profissionais de saúde mediante aprovação ética, aplicar revisão clínica independente aos itens ambíguos, comparar modelos locais maiores e mais recentes, e mensurar custo computacional, consumo de VRAM, latência e estabilidade sob carga. O cenário atual de hardware ainda limita a aplicação de LLMs locais em produção de baixo custo, especialmente quando há necessidade de baixa latência e modelos com maior número de parâmetros. Espera-se, contudo, que a evolução de modelos, técnicas de quantização, aceleração em hardware e métodos de RAG torne esse tipo de sistema progressivamente mais simples de implantar em ambientes assistenciais.

Por fim, a principal contribuição do protótipo é explicitar uma arquitetura replicável na qual privacidade, auditabilidade e revisão humana são requisitos de projeto. O estudo também fornece um benchmark inicial para avaliar recuperação, geração, privacidade e escopo em pré-natal, criando uma base para comparações futuras com modelos e configurações mais robustas.

% o profissional consegue valdiar a sugestao em segundos?
% profissionais conseguem revisar antes de salvar?
% como é reduzida a over-reliance na IA?
% como é calculado o impaco na carga cognitiva?
...
Como contribuição, o estudo entrega uma arquitetura replicável, um fluxo de privacidade coerente com os princípios da LGPD e um benchmark inicial reprodutível para avaliação técnica de recuperação, geração, mascaramento de PII, escopo e latência em documentos de pré-natal e evidências quantitativas sobre os trade-offs entre execução local e custo computacional.
A continuidade do projeto deve priorizar curadoria do índice vetorial, avaliação sistemática do STT, testes com modelos maiores e mais recentes, análise de consumo de VRAM e validação futura com profissionais de saúde em termos de eficácia assistencial e usabilidade em consulta mediante aprovação ética.
No cenário atual, os requisitos de hardware ainda limitam a adoção de baixo custo em produção; contudo, avanços em modelos, quantização, aceleração em hardware e técnicas de RAG podem tornar esse tipo de sistema progressivamente mais viável em ambientes reais.
