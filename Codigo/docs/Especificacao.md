...
| RF03 | Agente Escriba (STT) | O WebApp deve capturar áudio do microfone e processar a conversão *Speech-to-Text* (via motor offline **Faster-Whisper**) isolado da internet. O serviço `stt-service` aplica pré-processamento de áudio (normalização, high-pass) e expõe texto + segmentos temporais ao WebSocket; ver `docs/Escriba_STT.md`. |
| RF03a | Diarização de locutores (opcional) | O sistema pode atribuir rótulos de locutor (profissional/gestante) à transcrição via serviço offline dedicado **`diarization`** (`pyannote.audio` 3.1), 100% local (modelos cacheados em build, runtime `HF_HUB_OFFLINE=1`, CPU por padrão). Estratégia: diarização por segmento bufferizado (áudio efêmero do trecho → turnos mesclados com a transcrição), com rótulo de papel **editável pelo profissional** (*human-in-the-loop*). Opcional/desligado por padrão (`DIARIZATION_HTTP_URL` vazio); não bloqueia o fluxo. Ver `diarization/README.md`. |
| RF04 | Preenchimento Estruturado | O sistema deve injetar os resultados da transcrição mapeados diretamente nas variáveis e input-fields virtuais da caderneta utilizando o Agente LLM local, **sem persistir a transcrição bruta**. **Implementação:** a cada trecho clínico fechado (VAD/pontuação/debounce), o backend dispara em paralelo (1) sugestões Markdown PERGUNTA/CONDUTA/ALERTA e (2) extração estruturada via `POST /mcp/escriba/extract-fields-stream`; o front recebe `form_patch` no WebSocket para pré-preencher a aba Prontuário (efêmero até “Salvar rascunho”). Transcrição, insight e diarização **não** são persistidos (tabela `consulta_stream_evento` removida). |
...
| RNF04 | Segurança de Dados/LGPD (Anonymizer) | O backend deve aplicar um Proxy via **Servidor MCP** antes de bater no Agente Ollama. A camada deve interceptar a string limando o `nome_real`, Cartão SUS e NIS por tokens anonimizadores. A transcrição bruta do áudio **nunca deve ser persistida**. **Implementação:** `stt_partial`, `stt_diarized`, `ia_token` e `form_patch` são efêmeros no WebSocket; o `history` no reconnect é sempre vazio. |
