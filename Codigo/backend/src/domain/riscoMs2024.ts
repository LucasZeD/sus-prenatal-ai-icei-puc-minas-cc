/**
 * Estratificaùùo de risco obstùtrico (regras determinùsticas) alinhada ao
 * Guia de Atenùùo ù Saùde da Gestante ù Critùrios para estratificaùùo de risco
 * e acompanhamento da gestante (Ministùrio da Saùde, 2024).
 *
 * O guia prevù trùs nùveis (habitual, intermediùrio, alto). O produto persiste
 * apenas `HABITUAL` | `ALTO`; critùrios do guia em **intermediùrio** sùo
 * mapeados para `ALTO` (decisùo de produto: fila conservadora).
 *
 * Lacunas do DER em relaùùo ao guia (nùo computùveis sem novos campos/API):
 * - Escores de tabagismo (ex.: Fagerstrùm) e de ùlcool (ex.: T-ACE).
 * - Classificaùùo fina de sùfilis (terciùria / resistùncia) e estùgio clùnico.
 * - Muitos achados de exames laboratoriais (HIV, VDRL quantitativo, Hb, glicemia)
 *   enquanto nùo houver escrita estruturada na API clùnica.
 * - Condiùùes descritas sù em texto livre de consulta.
 *
 * Revisùo clùnica: conferir quadros do PDF e ajustar limiares/cùdigos conforme
 * versùo institucional em uso.
 */

import { RiscoGestacional } from "../lib/prismaBarrel.js";

/** Cùdigos estùveis para testes, auditoria e futura UI (tooltip). */
export type RiscoCriterioCodigo =
  // --- Alto risco (guia / manual de referùncia) ---
  ...
  // --- Intermediùrio no guia ? persistido como ALTO ---
  | "INT_IDADE_15_A_17"
  | "INT_IDADE_35_A_39"
  | "INT_IMC_30_A_39_9"
  | "INT_TABAGISMO"
...
function isGestacaoMultipla(tipoGravidez: string | null | undefined): boolean {
  ...
  return t.includes("gemel") || t.includes("tripla") || t.includes("mùltipl") || t.includes("multipl");
}
...
/**
 * Calcula `tipo_risco` e a lista de critùrios disparados (alto antes de intermediùrio).
 */
