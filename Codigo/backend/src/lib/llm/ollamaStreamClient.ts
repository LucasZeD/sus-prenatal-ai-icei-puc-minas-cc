...
import { llmConcurrencyLimiter } from "../concurrencyLimiter.js";
...
export class OllamaStreamClient {
  async *streamInsight(prompt: string): AsyncGenerator<string, void, undefined> {
    const base = normalizeHttpBase(process.env.OLLAMA_HTTP_URL);
    ...
    const release = llmConcurrencyLimiter.acquire();
    try {
